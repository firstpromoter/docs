...
All tools are scoped to the company you authorized during the OAuth consent step. Tools that accept `campaign_id` parameters are additionally filtered to campaigns your account has access to.

## Reports & analytics

| Tool | Description |
|---|---|
| `get_company_context` | Campaigns, currency, active/pending promoter counts. The entry point for orienting the model to your account. |
...
| `get_overview_report` | Revenue, referrals, customers, clicks, sales, refunds, and earnings grouped by day, week, month, or year. Accepts `start_date`, `end_date`, `group_by`, `campaign_id`. |
| `get_promoters_report` | Performance ranking per promoter for a date range. Accepts `start_date`, `end_date`, `campaign_id`, `sort_by`, `limit`. |
...
| `accept_promoter` | Accept a pending promoter, scoped to a campaign. Verifies the state change persisted before reporting success. |
| `reject_promoter` | Reject a pending promoter, scoped to a campaign. |
| `block_promoter` | Block an active promoter, scoped to a campaign. |
...
| `update_product` | Update name, external ID, or prices. |
...
| Tool | Description |
|---|---|
| `list_emails` | Filter by status (`draft`, `scheduled`, `sent`, `failed`) and trigger event (`promoter_accepted`, `lead_signup`, `reward_created`, etc.). Useful for mapping an existing onboarding sequence. |
| `get_email` | Full HTML body and trigger configuration for a single email. |
| `create_email` | Draft a new transactional email tied to a trigger event. Created in draft status with the trigger disabled — an admin must review and enable it from the dashboard before anything sends. Merge tags like `{{promoter.first_name}}` are supported. |
...
Money-movement operations are intentionally kept out of the MCP surface:

- Creating payment batches, paying out a batch, marking payouts paid/unpaid, deleting payouts
- Voiding, finalizing, or destroying invoices
- Generating payouts from commissions
- Creating, approving, or denying commissions
- Splitting commissions or importing previous sales from Stripe
- Bulk referral operations (approve/disable/enable/delete by query, move referrals to another promoter)
- Send-now, send-test, and broadcast email delivery
- Promoter archiving (restore is available; archive is not)

These remain dashboard-only. The AI helps you draft and configure; the human commits the money decisions.
